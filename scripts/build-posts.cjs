#!/usr/bin/env node
/**
 * Gen HTML cho toàn bộ bài từ Markdown.
 * Nguồn: md/*.md  →  bai/<slug>.html + index.html + posts.json
 */
const fs = require("fs");
...
function articleHtml(post) {
  const imgFile = post.coverImage.replace(/^img\//, "");
...
function main() {
  ...
  for (const post of posts) {
    ...
    const img = path.join(IMG_DIR, post.coverImage.replace(/^img\//, ""));
  ...
  console.log(`xong ${posts.length} bài`);
}
